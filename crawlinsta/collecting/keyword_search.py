    ...
    def fetch_data(self) -> None:
        ...
        if not self.pers:
            ...
            not_pers_btn = self.driver.find_element(
                By.XPATH,
                '//div[@aria-label="Not personalised" or @aria-label="Not personalized" or @aria-label="Nicht personalisiert"]'
                '[@role="button"][@tabindex="0"]'
                '//span[text()="Not personalised" or text()="Not personalized" or text()="Nicht personalisiert"]')
            not_pers_btn.click()
            time.sleep(random.SystemRandom().randint(6, 8))
